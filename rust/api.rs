...
use elements::{secp256k1_zkp, Address as LwkAddress, AddressParams, Script, Transaction};
...
impl Api {
    pub fn new_descriptor(network: Network, mnemonic: String) -> anyhow::Result<String, LwkError> {
    ...
    pub fn signed_pset_with_extra_details(
        ...
        Wallet::retrieve_wallet(wallet_id)?.signed_pset_with_extra_details(network, pset, mnemonic)
    }
}
...
mod test {
    ...
    fn test_api() {
        ...
        let _decoded = Api::decode_tx(wallet_id.clone(), pset.clone()).unwrap();
